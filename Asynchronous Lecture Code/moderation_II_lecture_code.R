...
mod_mod_probe <- data.frame(
  x = rep(c("Cooperative", "Competitive"), 6),
  w = rep(c("Low Performance", "Low Performance", 
            "Average Performance", "Average Performance", 
            "High Performance", "High Performance"), 2),
  z = c(rep("Sore Loser", 6), rep("Not Sore Loser", 6)),
  y = c(
    # Sore Loser panel (hate_lose = 0)
    4.3916, 4.0655,  # Low W  (Coop, Comp)
    4.4618, 3.9862,  # Avg W  (Coop, Comp)
    4.5320, 3.9070,  # High W (Coop, Comp)
    
    # Not Sore Loser panel (hate_lose = 1)
    4.3003, 4.9879,  # Low W  (Coop, Comp)
    4.5041, 4.2564,  # Avg W  (Coop, Comp)
    4.7079, 3.5249   # High W (Coop, Comp)
  )
)
# create the plot
